...
var estado_atual = DORMINDO
@export var delay : float = 0.04 
var timer : float = 0.0
...
func _physics_process(delta: float) -> void:
	...
	if not is_moving:
		timer += delta
		if timer >= delay:
			decidir_direcao_skull()
			timer = 0.0
			
	if jogador and global_position.distance_to(jogador.global_position) <= 16 and estado_atual == PERSEGUINDO:
		jogador.executar_morte()

func decidir_direcao_skull():
